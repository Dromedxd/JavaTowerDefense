...
public class Toolbar extends Bar {
	...
	// Añadimos botones aqui en la barra
	private MyButton bFill, bWater, bRoadS, bRoadC, water, mar;
	...
	private void initPathImgs() {
		pathStart = LoadSave.getSpriteAtlas().getSubimage(13 * 32, 18 * 32, 32, 32);
		PathEnd = LoadSave.getSpriteAtlas().getSubimage(14 * 32, 18 * 32, 32, 32);

	}

	private void initButtons() {

		bMenu = new MyButton("Menu", 2, 650, 100, 30);
		...
		bFill = new MyButton("Relleno", xStart, yStart, w, h, i++);
		bWater = new MyButton("Agua", xStart + xOffset, yStart, w, h, i++);
//		bRoadS = new MyButton("Carretera", xStart + xOffset , yStart, w, h, i++);
//		bRoadC = new MyButton("Giros", xStart + xOffset , yStart, w, h, i++);
		...
		initMapButton(water, editing.getGame().getTileManager().getWater(), xStart, yStart, xOffset, w, h, i++);
		initMapButton(mar, editing.getGame().getTileManager().getMar(), xStart, yStart, xOffset, w, h, i++);
...
//		bPathEnd.draw(g);
	...
	public void mouseClicked(int x, int y) {
		if (bMenu.getBounds().contains(x, y))
			...
			editing.setSelectedTile(selectedTile);
			return;
		} else if (bFill.getBounds().contains(x, y)) {
			selectedTile = editing.getGame().getTileManager().getTile(bFill.getId());
			editing.setSelectedTile(selectedTile);
			return;
		}
